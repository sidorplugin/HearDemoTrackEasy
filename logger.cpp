...
//  ласс LogStreamPrivate нужен только дл€ вызова конструктора
// LogStream(const QString& logFileName, bool append),
// поскольку в этом случае нужно хранить указатель на файл куда производитс€
// запись. ¬ других случа€х это не нужно.
...
// ќн конечно удобен, но все тоже можно реализовать и с помощью
...
// ----------------------------------------------------------------------------
// LogEntry используетс€ вместо обычного указател€ на LogStream, поскольку
// первоначально была иде€, что будут хранитьс€ еще какие-то дополнительные
// параметры.
...
public:
    ...
    // logsNames нужен дл€ удобства.
    // »спользуетс€ дл€ более эффективной работы getStream(unsigned int index)
...
// ћакрос дл€ вставки новых элементов.  ѕоскольку создание объектов LogStream
// производитьс€ нами, то перед тем как  вставл€ть новый нужно правильно убрать
// старый дл€ того, чтобы избежать утечек пам€ти.
